...
# get Bright Star Catalogue -> pandas dataframe
def get_BSC_cat(filename='catalog'):
    ...
    for i in range(len(catalogue_raw)):
        try:
            ra = int(catalogue_raw['RAh'][i]) + int(catalogue_raw['RAm'][i]) / 60 + float(
                catalogue_raw['RAs'][i]) / 3600
            ra *= 15  # to deg
            ra *= np.pi / 180  # to rad
            dec = int(catalogue_raw['DEd'][i]) + int(catalogue_raw['DEm'][i]) / 60 + float(
                catalogue_raw['DEs'][i]) / 3600
...
# get HIP catalogue -> pandas dataframe
def get_HIP_cat(filename='hip_main.dat'):
...
# get HIP+BSC csv catalogue, use HIP value
def get_Merge_cat(filename='myHIP+BSC.csv'):
...
# get BSC-update csv catalogue, use BSC/HIP value
# warn: myBSCupdate.csv missed some bright stars -> buggy
def get_BSCupdate_cat(filename='myBSCupdate1arcmin.csv'):
    cat_df = pd.read_csv(filename)
...
def get_RGB_list(bv_list, default_teff=5700, teff_filename='TCTable.csv'):
...
# [88 constellations] -> [lines of a con] -> [ID of stars in a line]
def get_cons_ids(filename='cons_lines.txt',
                 catalogue_df=None):
...
def get_cons_names(filename='cons_lines.txt', full_name=False):
    list_of_names = []
    with open(filename, 'r', encoding='utf-8') as f:
        constellation = None
        for s_line in f.readlines():
...
def move_cat_df(cat_df, time_year=0, ra_deg=0, dec_deg=0, travel_ly=0,
    ...
    mag_new[mag_new < min_mag] = 20  # set to negligible if too close
    ...
    # new pmRA, pmDE, RadVel is calculate-able theoretically
